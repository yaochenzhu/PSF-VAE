...
class CollaborativeVariationalAutoencoder():
    '''
       	The collaborative variational autoencoder
    '''
    def __init__(self, 
                 input_dim,
                 hidden_sizes,
    ...
    def build_split_gens(self, dim1, logits=True):
        '''
            Split the latent variable into two parts
        '''
        if not hasattr(self, "vae_split_gens"):
            dim2 = self.latent_size - dim1
            z1_in = layers.Input(shape=[dim1, ], name="latent_first")
            z2_in = layers.Input(shape=[dim2, ], name="latent_second")
            gen_dense1 = self.decoder.dense_list[0]
            gen_dense_rems = self.decoder.dense_list[1:]
            activation = None if logits else self.decoder_activs[0]
            sub_dense11 = SubDense(weights=gen_dense1.weights, 
                                   activation=activation,
                                   s_idx=0, e_idx=dim1,
                                   name="split_gen_dense11")  
            sub_dense12 = SubDense(weights=gen_dense1.weights,
                                   activation=activation,
                                   s_idx=dim1, e_idx=-1,
                                   name="split_gen_dense12") 
            r1_out = sub_dense11(z1_in)
            r2_out = sub_dense12(z2_in)
            for dense in gen_dense_rems:
                r1_out = dense(r1_out)
                r2_out = dense(r2_out)
            self.vae_gen1 = models.Model(inputs=z1_in, outputs=r1_out)
            self.vae_gen2 = models.Model(inputs=z2_in, outputs=r2_out)
            self.vae_split_gens = [self.vae_gen1, self.vae_gen2]
        return self.vae_split_gens
